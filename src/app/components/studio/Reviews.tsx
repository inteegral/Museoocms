...
export function Reviews() {
  ...
  const filteredReviews = reviews.filter((r) => {
    ...
    const matchesGuide = filterGuide === "all" || r.guideName === filterGuide;
  ...
  const guideStats = guideNames.map((name) => {
    ...
    return { name, avg: (gr.reduce((s, r) => s + r.rating, 0) / gr.length).toFixed(1), count: gr.length };
  });
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            <div className="size-10 rounded-xl flex items-center justify-center bg-amber-50">
              <MessageSquare className="size-5 text-amber-600" />
            </div>
            <div>
              <h1 className="text-[22px] font-semibold text-zinc-900 tracking-tight">Reviews</h1>
              <p className="text-[13px] text-zinc-400 mt-0.5">{reviews.length} reviews · {averageRating} avg rating</p>
            </div>
          </div>
        </div>
        ...
        {filterGuide !== "all" && (
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setFilterGuide("all")} className="flex items-center gap-1 text-[12px] text-zinc-400 hover:text-zinc-700 transition-colors">
              <X className="size-3.5" /><span>Clear</span>
            </button>
          </div>
        )}
        ...
        <div className="mb-6 bg-white border border-zinc-200 rounded-xl overflow-hidden" style={{ boxShadow: "0 2px 8px 0 rgba(0,0,0,0.06)" }}>
          ...
          >
            ...
            {insightsOpen ? <ChevronUp className="size-4 text-zinc-400" /> : <ChevronDown className="size-4 text-zinc-400" />}
          </button>
          {insightsOpen && (
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-zinc-200 rounded-lg text-[13px] text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900"
              />
            </div>
            <div className="relative">
              <select
                ...
                className="appearance-none pl-4 pr-8 py-2.5 bg-white border border-zinc-200 rounded-lg text-[13px] font-medium text-zinc-700 focus:outline-none focus:ring-2 focus:ring-zinc-900 cursor-pointer"
              >
        ...
        {/* Reviews list */}
        <div className="bg-white border border-zinc-200 rounded-xl px-5" style={{ boxShadow: "0 1px 3px 0 rgba(0,0,0,0.04)" }}>
