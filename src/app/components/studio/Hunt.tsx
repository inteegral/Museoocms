...
function genToken(poiId: string) {
  return `QR-${poiId.toUpperCase().replace("-", "")}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
}
...
const INITIAL_HUNTS: Hunt[] = [
  {
    ...
    questions: [
      ...
      },
      {
        ...
        correct: 1,
        unlockMode: "qr",
        qrToken: "QR-POI3-B8K2",
      },
    ],
    completionLog: [
      { id: "c-1", visitorName: "Marco R.",  score: 3, totalQuestions: 3, timeMs: 482000, completedAt: "2026-03-28" },
      { id: "c-2", visitorName: "Anna M.",   score: 3, totalQuestions: 3, timeMs: 445000, completedAt: "2026-03-31" },
      { id: "c-3", visitorName: "Sarah K.",  score: 3, totalQuestions: 3, timeMs: 534000, completedAt: "2026-03-29" },
      { id: "c-4", visitorName: "Luca B.",   score: 2, totalQuestions: 3, timeMs: 612000, completedAt: "2026-03-30" },
      { id: "c-5", visitorName: "Tom W.",    score: 1, totalQuestions: 3, timeMs: 780000, completedAt: "2026-04-01" },
    ],
...
// ── Unlock mode badge ─────────────────────────────────────────────────────────
function UnlockBadge({ mode }: { mode: HuntQuestion["unlockMode"] }) {
  if (mode === "qr")
    return <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-50 text-blue-600 border border-blue-100"><QrCode className="size-2.5" />QR</span>;
  if (mode === "geofence")
    return <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-violet-50 text-violet-600 border border-violet-100"><MapPin className="size-2.5" />GPS</span>;
  return <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-zinc-100 text-zinc-500 border border-zinc-200">Manual</span>;
}

// ── Leaderboard Modal ─────────────────────────────────────────────────────────
function LeaderboardModal({ hunt, onClose }: { hunt: Hunt; onClose: () => void }) {
  const sorted = [...hunt.completionLog].sort((a, b) =>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            <Medal className="size-4" style={{ color: "#D33333" }} />
            <span className="text-[14px] font-semibold text-zinc-900">{hunt.title}</span>
          </div>
        ...
        <div className="grid grid-cols-3 divide-x divide-zinc-100 border-b border-zinc-100">
          {[
            { label: "Completions", value: hunt.completions },
            { label: "Perfect scores", value: hunt.completionLog.filter(c => c.score === c.totalQuestions).length },
            { label: "Questions", value: hunt.questions.length },
          ].map(({ label, value }) => (
            <div key={label} className="px-4 py-3 text-center">
              <p className="text-[18px] font-semibold text-zinc-900">{value}</p>
        ...
        <div className="p-4 max-h-72 overflow-y-auto">
          {sorted.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-[13px] text-zinc-400">No completions yet</p>
            </div>
          ) : (
            <div className="space-y-2">
              {sorted.map((entry, i) => (
                <div
                  key={entry.id}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl ${i < 3 ? "bg-amber-50/60" : "bg-zinc-50"}`}
                >
                  <div
                    className="size-6 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold text-white"
                    style={{ background: i < 3 ? medalColors[i] : "#d4d4d8" }}
                  >
                    {i + 1}
                  </div>
                  <span className="flex-1 text-[13px] font-medium text-zinc-800">{entry.visitorName}</span>
                  <span className="text-[11px] font-semibold text-zinc-500 tabular-nums">
                    {entry.score}/{entry.totalQuestions}
                  </span>
                  <span className="text-[11px] text-zinc-400 tabular-nums flex items-center gap-0.5">
      ...
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 mb-2">
            {(["qr", "geofence", "manual"] as const).map(mode => (
              <button
                key={mode}
                type="button"
                onClick={() => {
                  const update: Partial<HuntQuestion> = { unlockMode: mode };
                  if (mode === "qr" && !q.qrToken) update.qrToken = genToken(q.poiId);
                  onChange({ ...q, ...update });
                }}
                className="px-2.5 py-1 rounded-md text-[11px] font-semibold transition-all"
                style={
                  q.unlockMode === mode
                    ? { background: "#D33333", color: "white" }
                    : { background: "white", color: "#71717a", border: "1.5px solid #e4e4e7" }
                }
              >
                {mode === "qr" ? "QR Scan" : mode === "geofence" ? "Geofence" : "Manual"}
              </button>
            ))}
          </div>
          ...
          {q.unlockMode === "qr" && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-zinc-50 border border-zinc-200">
              <QrCode className="size-3.5 text-zinc-400 flex-shrink-0" />
              <span className="text-[12px] font-mono text-zinc-600 flex-1">{q.qrToken}</span>
              <button
                type="button"
                onClick={() => onChange({ ...q, qrToken: genToken(q.poiId) })}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input
                  ...
                  step="0.0001"
                  value={q.lat ?? ""}
                  onChange={e => onChange({ ...q, lat: e.target.value ? parseFloat(e.target.value) : undefined })}
                  placeholder="e.g. 41.9028"
                  className="w-full px-3 py-2 rounded-lg border border-zinc-200 text-[12px] font-mono text-zinc-700 placeholder:text-zinc-300 focus:outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  step="0.0001"
                  value={q.lng ?? ""}
                  onChange={e => onChange({ ...q, lng: e.target.value ? parseFloat(e.target.value) : undefined })}
          ...
          {q.unlockMode === "manual" && (
            <p className="text-[11px] text-zinc-400">Visitors tap "I'm here" in the app to unlock this step.</p>
          )}
        </div>
      </div>
    </div>
...
function HuntEditor({
  ...
  const [timeLimitMinutes, setTimeLimitMinutes] = useState<number | undefined>(hunt?.timeLimitMinutes);
  ...
  const handleSave = () => {
    const saved: Hunt = {
      ...
      timeLimitMinutes,
      questions,
      completionLog: hunt?.completionLog ?? [],
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-1 space-y-4">
            <div className="bg-white border border-zinc-200 rounded-xl p-5 space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-semibold text-zinc-400 uppercase tracking-wide mb-1.5">
                  Time limit <span className="font-normal normal-case text-zinc-300">optional</span>
                </label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    ...
                    className="flex-1 px-3 py-2.5 rounded-lg border border-zinc-200 text-[13px] text-zinc-800 placeholder:text-zinc-300 focus:outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100"
                  />
                  <span className="text-[12px] text-zinc-400 flex-shrink-0">min</span>
                </div>
              </div>
            </div>
...
export function Hunt() {
  ...
  const [leaderboardHunt, setLeaderboardHunt] = useState<Hunt | null>(null);
  ...
  return (
    <PageShell>
      <div className="max-w-5xl mx-auto">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    ...
                    {h.timeLimitMinutes && (
                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-zinc-100 text-zinc-500 border border-zinc-200">
                        <Clock className="size-2.5" />{h.timeLimitMinutes} min
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-3 text-[12px] text-zinc-400 flex-wrap">
                    ...
                    <span className="flex items-center gap-1">
                      {Array.from(new Set(h.questions.map(q => q.unlockMode))).map(mode => (
                        <UnlockBadge key={mode} mode={mode} />
                      ))}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    title="Leaderboard"
                  >
                    <Medal className="size-3.5 text-zinc-500" />
                  </button>
        ...
        {leaderboardHunt && (
          <LeaderboardModal
            hunt={leaderboardHunt}
            onClose={() => setLeaderboardHunt(null)}
          />
        )}
      </div>
    </PageShell>
