    ...
    <div className="absolute inset-0 z-20 flex flex-col" style={{ top: 48 }}>
        ...
        {POI_FLOOR.map((poi, i) => (
          <button
            ...
            style={{
              ...
              zIndex: 8, border: "none", background: "transparent", padding: 0, cursor: "pointer",
            }}
          >
            <div className="flex flex-col items-center" style={{ filter: "drop-shadow(0 3px 10px rgba(211,51,51,0.45))" }}>
              <div style={{
                width: 36, height: 36, borderRadius: "50%",
                background: "#D33333", border: "2.5px solid white",
      ...
      {nearest && (
        <div className="absolute left-3 right-3 z-30 flex items-center gap-3 px-4 py-3 rounded-2xl shadow-xl" style={{ bottom: 16, background: "rgba(255,255,255,0.97)", backdropFilter: "blur(20px)", border: "1px solid rgba(211,51,51,0.12)" }}>
          ...
          <button onClick={() => onSelectPOI(nearestIdx)} className="px-3 py-1.5 text-white text-[11px] font-bold rounded-xl flex-shrink-0" style={{ background: "#D33333", fontFamily: "Inter, system-ui, sans-serif" }}>Play</button>
        </div>
      )}
    </div>
  );
...
export function GuidePreviewModal({ guideName, onClose }: GuidePreviewModalProps) {
  ...
  const selectPOIFromMap = (index: number) => {
    setShowMap(false);
    setActivePOIIndex(index);
    setProgress(0);
    setTimeout(() => {
      const el = poiRefs.current[index];
      if (el && scrollRef.current) {
        const containerRect = scrollRef.current.getBoundingClientRect();
        const elRect = el.getBoundingClientRect();
        scrollRef.current.scrollTo({ top: elRect.top - containerRect.top + scrollRef.current.scrollTop - 8, behavior: "smooth" });
      }
    }, 100);
  };
